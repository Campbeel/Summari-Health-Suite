...
- **2026-03-13**: Added PDF document generation for consultation records using PDFKit (`server/pdf-generator.ts`). Branded A4 PDFs with logo, doctor/patient info, prescription tables, medical instructions with priority badges, and exam order lists. API: GET `/api/consultations/:id/documents/pdf?types=prescription,instructions,exams` (accessible by both doctor and patient of the consultation). Email delivery now attaches PDF documents instead of inline HTML content. Frontend: "Descargar PDF" button in consultation summary dialog and standalone summary page. UX improvements: appointment cards are fully clickable, "Ver detalles" uses outline button style, consultation summary opens as modal dialog, prescription-paper dark mode fix.
