...
- **Roles & Organizations**: Strict role separation via `users.role` (`patient` | `doctor` | `admin` | `superAdmin`). `organizations` table holds clinic/group entities; `users.organization_id` and `doctors.organization_id` link members. Doctors may be independent (no org) or org-affiliated. `requireRole(...roles)` middleware in `server/auth.ts` gates endpoints. `useRole()` hook (`client/src/hooks/use-role.ts`) drives the sidebar menu and route gating; `ProtectedRoute` accepts `allowedRoles` and redirects via `homeForRole(role)`. Demo accounts: doctor 19684371-K/admin (Clínica Demo), patient 19892370-2/felix123, org admin 11111111-1/admin (Clínica Demo), super admin 99999999-9/admin.
- **Admin Layer (Org Admin)**: Pages at `/admin/dashboard`, `/admin/users`, `/admin/schedules`. Backend endpoints (all `requireRole("admin")`): `GET /api/admin/me/organization`, `GET /api/admin/stats` (revenue, scheduled/paid/lost/completed counts, per-doctor breakdown, scoped to org doctors), `GET /api/admin/users` (org members with doctor profile data), `GET /api/admin/doctors`, `POST /api/admin/doctors` (creates user + doctor profile in org), `PUT /api/admin/doctors/:id`, `PUT /api/admin/doctors/:id/availability`, `DELETE /api/admin/users/:id`. `/api/admin/check` returns `{ isAdmin, role, organizationId }`.
- **SuperAdmin Layer**: Pages at `/super-admin/dashboard` and `/super-admin/organizations`. Backend endpoints (`requireRole("superAdmin")`): `GET/POST /api/super-admin/organizations`, `PUT /api/super-admin/organizations/:id`, `DELETE /api/super-admin/organizations/:id` (soft-delete via `isActive=false`), `POST /api/super-admin/organizations/:id/admins` (create org admin user), `GET /api/super-admin/stats` (platform totals).
...
- **Prescription Safety**: Real-time allergy contraindication check on the validation page. For each medication, the doctor's typed name is normalized (lowercase + diacritic-stripped) and bidirectionally substring-matched (min length 3) against `patients.allergies`. On match a destructive inline alert shows the conflicting allergies and a mandatory acknowledgement checkbox ("…bajo mi responsabilidad…"). Without acknowledgement, "Validar y guardar" is intercepted and a destructive toast names the offending medication and allergies. Acknowledgement is reset automatically when the medication name is changed. When transcription was processed but the prescription remained empty, an amber banner explains the ambiguity ("No se detectaron medicamentos en la transcripción…").
- **Doctor Appointment Cancellation UX**: The cancel action on `/doctor/appointments` is a labeled "Cancelar cita" button (not a bare X icon) and opens an `AlertDialog` showing patient name, date and time before requiring explicit confirmation ("Sí, cancelar cita" vs "Conservar cita"). Prevents accidental destructive cancellations.
- **Public Doctor Listing DTO**: `GET /api/doctors` is intentionally public (used by the booking page before login) but `storage.getAllDoctors()` only returns display fields: `id`, `specialty`, `bio`, `consultationFee`, `availability`, `userName`, `userImage`. Sensitive fields like `userId` and `isActive` are not exposed.
